...
from CommonsCloudAPI.models.organization import Organization as Model
...
class Seed(Pod):
  ...
  def preprocessor_put_single(data=None, **kw):
    logger.debug('`Organization` preprocessor_put_single')
    authorization = verify_authorization()
    role = verify_roles(authorization, ['admin'])

  def preprocessor_put_many(**kw):
    logger.debug('`Organization` preprocessor_put_many')
    authorization = abort(403)

  def preprocessor_patch_single(data=None, **kw):
    logger.debug('`Organization` preprocessor_patch_single')
    authorization = verify_authorization()
    role = verify_roles(authorization, ['admin'])
